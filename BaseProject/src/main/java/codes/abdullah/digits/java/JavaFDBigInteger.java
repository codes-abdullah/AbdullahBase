 ...
 */
public /*@ spec_bigint_math @*/ class JavaFDBigInteger {
    ...
    private static final JavaFDBigInteger POW_5_CACHE[];

    // Initialize FDBigInteger cache of powers of 5.
    static {
        POW_5_CACHE = new JavaFDBigInteger[MAX_FIVE_POW];
        int i = 0;
        while (i < SMALL_5_POW.length) {
            JavaFDBigInteger pow5 = new JavaFDBigInteger(new int[]{SMALL_5_POW[i]}, 0);
            pow5.makeImmutable();
            POW_5_CACHE[i] = pow5;
            i++;
        }
        JavaFDBigInteger prev = POW_5_CACHE[i - 1];
    ...
    public static final JavaFDBigInteger ZERO = new JavaFDBigInteger(new int[0], 0);
    ...
    /*@
     ...
     @*/
    private JavaFDBigInteger(int[] data, int offset) {
        this.data = data;
        this.offset = offset;
    ...
    /*@
     ...
     @*/
    public JavaFDBigInteger(long lValue, char[] digits, int kDigits, int nDigits) {
    ...
    /*@
     ...
     @*/
    public static JavaFDBigInteger valueOfPow52(int p5, int p2) {
        if (p5 != 0) {
            if (p2 == 0) {
                return big5pow(p5);
                ...
                if (bitcount == 0) {
                    return new JavaFDBigInteger(new int[]{pow5}, wordcount);
                } else {
                    return new JavaFDBigInteger(new int[]{
                            pow5 << bitcount,
                            pow5 >>> (32 - bitcount)
                    }, wordcount);
                }
    ...
    /*@
     ...
     @*/
    public static JavaFDBigInteger valueOfMulPow52(long value, int p5, int p2) {
        assert p5 >= 0 : p5;
        assert p2 >= 0 : p2;
        int v0 = (int) value;
        int v1 = (int) (value >>> 32);
        ...
        if (p5 != 0) {
            if (p5 < SMALL_5_POW.length) {
                ...
                int v2 = (int) (carry >>> 32);
                if (bitcount == 0) {
                    return new JavaFDBigInteger(new int[]{v0, v1, v2}, wordcount);
                } else {
                    return new JavaFDBigInteger(new int[]{
                            v0 << bitcount,
                ...
                JavaFDBigInteger pow5 = big5pow(p5);
                int[] r;
                if (v1 == 0) {
                ...
                return (new JavaFDBigInteger(r, pow5.offset)).leftShift(p2);
            }
        } else if (p2 != 0) {
            if (bitcount == 0) {
                return new JavaFDBigInteger(new int[]{v0, v1}, wordcount);
            } else {
                return new JavaFDBigInteger(new int[]{
                         v0 << bitcount,
                        (v1 << bitcount) | (v0 >>> (32 - bitcount)),
        ...
        return new JavaFDBigInteger(new int[]{v0, v1}, 0);
    }
    ...
    /*@
     ...
     @*/
    private static JavaFDBigInteger valueOfPow2(int p2) {
        int wordcount = p2 >> 5;
        int bitcount = p2 & 0x1f;
        return new JavaFDBigInteger(new int[]{1 << bitcount}, wordcount);
    }
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() << shift);
     @*/
    public JavaFDBigInteger leftShift(int shift) {
        if (shift == 0 || nWords == 0) {
            return this;
        }
        ...
        if (this.isImmutable) {
            if (bitcount == 0) {
                return new JavaFDBigInteger(Arrays.copyOf(data, nWords), offset + wordcount);
            } else {
                ...
                return new JavaFDBigInteger(result, offset + wordcount);
            }
        } else {
            if (bitcount != 0) {
                int anticount = 32 - bitcount;
    ...
    /*@
     ...
     @*/
    public int quoRemIteration(JavaFDBigInteger S) throws IllegalArgumentException {
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() * 10);
     @*/
    public JavaFDBigInteger multBy10() {
        if (nWords == 0) {
            return this;
        }
        if (isImmutable) {
            ...
            return new JavaFDBigInteger(res, offset);
        } else {
            int p = multAndCarryBy10(this.data, this.nWords, this.data);
            if (p != 0) {
                if (nWords == data.length) {
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() * pow52(p5, p2));
     @*/
    public JavaFDBigInteger multByPow52(int p5, int p2) {
        if (this.nWords == 0) {
            return this;
        }
        JavaFDBigInteger res = this;
        if (p5 != 0) {
            int[] r;
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() - subtrahend.value());
     @*/
    public JavaFDBigInteger leftInplaceSub(JavaFDBigInteger subtrahend) {
        assert this.size() >= subtrahend.size() : "result should be positive";
        JavaFDBigInteger minuend;
        if (this.isImmutable) {
            minuend = new JavaFDBigInteger(this.data.clone(), this.offset);
        } else {
            minuend = this;
        }
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() - subtrahend.value());
     @*/
    public JavaFDBigInteger rightInplaceSub(JavaFDBigInteger subtrahend) {
        assert this.size() >= subtrahend.size() : "result should be positive";
        JavaFDBigInteger minuend = this;
        if (subtrahend.isImmutable) {
            subtrahend = new JavaFDBigInteger(subtrahend.data.clone(), subtrahend.offset);
        }
    ...
    /*@
     ...
     @*/
    public /*@ pure @*/ int cmp(JavaFDBigInteger other) {
    ...
    /*@
     ...
     @*/
    public /*@ pure @*/ int addAndCmp(JavaFDBigInteger x, JavaFDBigInteger y) {
        JavaFDBigInteger big;
        JavaFDBigInteger small;
        int xSize = x.size();
        int ySize = y.size();
        int bSize;
        int sSize;
    ...
    /*@
     ...
     @*/
    private JavaFDBigInteger mult(int i) {
        if (this.nWords == 0) {
            return this;
        }
        int[] r = new int[nWords + 1];
        mult(data, nWords, i, r);
        return new JavaFDBigInteger(r, offset);
    }
    ...
    /*@
     ...
     @*/
    private JavaFDBigInteger mult(JavaFDBigInteger other) {
        if (this.nWords == 0) {
            return this;
        }
        if (this.size() == 1) {
        ...
        return new JavaFDBigInteger(r, this.offset + other.offset);
    }
    ...
    /*@
     ...
     @ ensures \result.value() == \old(this.value() + other.value());
     @*/
    private JavaFDBigInteger add(JavaFDBigInteger other) {
        JavaFDBigInteger big, small;
        int bigLen, smallLen;
        int tSize = this.size();
        ...
        r[bigLen] = (int) carry;
        return new JavaFDBigInteger(r, 0);
    }
    ...
    /*@
     ...
     @ ensures \result == 0;
     @*/
    private /*@ helper @*/ long multDiffMe(long q, JavaFDBigInteger S) {
        long diff = 0L;
        if (q != 0) {
     ...
     */
    private static JavaFDBigInteger big5pow(int p) {
     ...
     * @return <code>5<sup>p</sup></code>.
     */
    private static JavaFDBigInteger big5powRec(int p) {
        if (p < MAX_FIVE_POW) {
            return POW_5_CACHE[p];
        }
        // construct the value.
        ...
        q = p >> 1;
        r = p - q;
        JavaFDBigInteger bigq = big5powRec(q);
