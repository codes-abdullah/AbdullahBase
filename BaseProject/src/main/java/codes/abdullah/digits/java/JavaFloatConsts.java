...
public class JavaFloatConsts {
	/**
	 * Don't let anyone instantiate this class.
	 */
	private JavaFloatConsts() {
	}
	...
	/**
	 * The number of logical bits in the significand of a <code>float</code> number,
	 * including the implicit bit.
	 */
	public static final int SIGNIFICAND_WIDTH = 24;

	/**
	 * Maximum exponent a finite <code>float</code> number may have. It is equal to
	 * the value returned by <code>Math.ilogb(Float.MAX_VALUE)</code>.
	 */
	public static final int MAX_EXPONENT = 127;

	/**
	 * Minimum exponent a normalized <code>float</code> number may have. It is equal
	 * to the value returned by <code>Math.ilogb(Float.MIN_NORMAL)</code>.
	 */
	...
	static {
		...
		assert (((SIGN_BIT_MASK | EXP_BIT_MASK | SIGNIF_BIT_MASK) == ~0) && (((SIGN_BIT_MASK & EXP_BIT_MASK) == 0)
				&& ((SIGN_BIT_MASK & SIGNIF_BIT_MASK) == 0) && ((EXP_BIT_MASK & SIGNIF_BIT_MASK) == 0)));
	}
}
